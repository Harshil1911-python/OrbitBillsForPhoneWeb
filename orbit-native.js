(function(){
  ...
  function ensureOfflineModal(){
    ...
    wrap.innerHTML='<div role="dialog" style="width:100%;max-width:340px;background:#fff;border-radius:16px;padding:22px 18px 16px;box-shadow:0 20px 50px rgba(15,23,42,.25);font-family:system-ui,sans-serif;">'
      +'<h3 style="margin:0 0 8px;font-size:17px;font-weight:700;color:#101a2b;">You are offline</h3>'
      +'<p style="margin:0 0 14px;font-size:14px;line-height:1.45;color:#5b6b82;">Data stays on this device. You can keep billing as usual.</p>'
      +'<div id="orbitOffLearn" style="display:none;margin:0 0 14px;padding:12px;border-radius:12px;background:#f5f8fe;border:1px solid #dfe7f5;font-size:13.5px;line-height:1.45;">'
      +'<strong style="display:block;margin-bottom:4px;">Latest features may not work in offline mode</strong>'
      +'When offline the app uses pages stored inside it. Updates appear next time you open online.</div>'
      +'<button type="button" id="orbitOffLearnBtn" style="width:100%;margin-bottom:8px;min-height:44px;border:1px solid #dfe7f5;background:#fff;color:#0b3d91;font-weight:700;font-size:14px;border-radius:11px;">Learn more</button>'
      +'<button type="button" id="orbitOffOk" style="width:100%;min-height:44px;border:0;background:#0b3d91;color:#fff;font-weight:700;font-size:14px;border-radius:11px;">OK</button></div>';
    (document.body||document.documentElement).appendChild(wrap);
    wrap.addEventListener("click",function(e){ if(e.target===wrap) hideOfflineModal(); });
    var learnBtn=document.getElementById("orbitOffLearnBtn"), learnBox=document.getElementById("orbitOffLearn");
    if(learnBtn&&learnBox) learnBtn.addEventListener("click",function(){
      var open=learnBox.style.display==="block"; learnBox.style.display=open?"none":"block"; learnBtn.textContent=open?"Learn more":"Hide details";
    });
  ...
  function setupNetwork(){
    ...
    if(!bar){
      ...
      x.style.cssText="position:absolute;right:8px;top:50%;transform:translateY(-50%);width:32px;height:32px;border:0;background:transparent;color:#fff;font:700 22px/1 system-ui,sans-serif;";
      x.addEventListener("click",function(){ try{ sessionStorage.setItem("orbit_offline_banner_dismissed","1"); }catch(e){} bar.style.display="none"; bar.setAttribute("data-dismissed","1"); });
    ...
    function setOnline(ok){
      if(ok){ bar.style.display="none"; try{ sessionStorage.removeItem("orbit_offline_banner_dismissed"); }catch(e){} bar.removeAttribute("data-dismissed");
        try{ if(hasCap()&&PREFER_LIVE&&!isOnLiveHost()&&(isLocalCapOrigin()||location.protocol==="file:")&&sessionStorage.getItem("orbit_live_redirected")!=="1") tryHybridLiveRedirect(); }catch(e){} return; }
      if(!isDismissed()) bar.style.display="block"; try{ showOfflineModal(); }catch(e){}
    }
  ...
  function setupBackButton(){
    ...
    window.__orbitAndroidBack=function(){
      ...
      var openModal=document.querySelector(".modal-bg.open"); if(openModal){ openModal.classList.remove("open"); return true; }
  ...
  /** Share blob with Android system sheet — attaches file for WhatsApp / Drive / Gallery */
  window.__orbitNativeShare=async function(opts){
    opts=opts||{};
    var title=opts.title||"OrbitBills", text=opts.text||"", filename=opts.filename||"file.bin", blob=opts.blob||null, dialogTitle=opts.dialogTitle||"Share";
    if(!blob) return false;

    // A) Capacitor Share + Filesystem (best for APK)
    try{
      if(hasCap()){
        var Share=plugin("Share"), Filesystem=plugin("Filesystem");
        if(Share&&Share.share&&Filesystem&&Filesystem.writeFile){
          ...
          if(uri){
            try{ await Share.share({title:title,text:text,url:uri,dialogTitle:dialogTitle}); return true; }
            catch(e1){ try{ await Share.share({title:title,text:text,files:[uri],dialogTitle:dialogTitle}); return true; }catch(e2){} }
          }
        }
      }
    }catch(eCap){ console.warn("native share cap", eCap); }

    // B) Web Share API with File (Chrome Android / WebView)
    try{
      var mime=blob.type||opts.mime||"application/octet-stream";
      var file=null; try{ file=new File([blob],filename,{type:mime,lastModified:Date.now()}); }catch(e){}
      if(file&&navigator.share){
        ...
        try{ await navigator.share({files:[file],title:title,text:text}); return true; }
        catch(eS2){ if(eS2&&eS2.name==="AbortError") return true; }
      }
    }catch(eW){}

    // C) text-only
    try{ if(navigator.share){ await navigator.share({title:title,text:text}); return true; } }catch(eT){ if(eT&&eT.name==="AbortError") return true; }
    return false;
  };
